...
- **원인을 세 갈래로 가른다.** 처방이 서로 다르기 때문이다:
  ...
  ③ 상태를 바꾸는 명령(**넣지 않는다** — 묻는 것이 맞다)
- **③을 자동 제안하지 않는다.** "승인 요청을 줄인다"는 목표는 구조적으로 허용 목록을 넓히는
  쪽으로 기운다. 이 템플릿은 정반대 조항을 갖고 있어(CLAUDE.md '허용 목록을 넓히지 않는다')
  안전한 쪽으로만 제안하도록 막아 둔다.
...
def allowed(segment: str, rules: list[str], extra: list[str] = ()) -> bool:
    ...
    for rule in list(rules) + list(extra):
        if rule.endswith(" *"):
            if segment == rule[:-2] or segment.startswith(rule[:-1]):
                return True
        elif segment == rule:
            return True
    return False
...
def main() -> int:
    ...
    labels = {
        ...
        "상태변경": "상태를 바꾸는 명령 (넣지 않는다 — 묻는 것이 맞다)",
        "판단필요": "분류 밖 (사람이 판단)",
    }
    ...
    print("  2. 규칙을 넣었으면 **세션을 재시작**해야 적용된다(settings.json은 시작 시 읽힌다).")
    print("  3. ★ 위 수치는 접두사 매처의 **예측**이다. 재시작 후 다시 돌려 실제로 줄었는지 확인해라.")
    return 0
